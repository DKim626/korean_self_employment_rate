...
# - Define functions to load raw .csv files
# - Hook utils functions to overide KOSIS .R code
# - Save the results in .rds format
#
#----------------------------------------------------------------------------- -
#
# Usage:
# - Source in preprocess.R
# - Functions:
#   - extract_code_list(raw_code_dir):
#     - Extracts code file list in 'code/raw'
#   - run_kosis_script(file, raw_data_dir, new_env):
#     - Runs KOSIS codes automatically with encoding CP949
#   - save_processed(survey_year, data_dir, new_env)
#     - Save the processed files into .rds form
#
#----------------------------------------------------------------------------- -
#
# Notes:
...
extract_code_list <- function(raw_code_dir) {
  ...
  code_list <- code_list[!stri_detect_regex(code_list, '/1986')]
